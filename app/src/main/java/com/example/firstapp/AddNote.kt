...
class AddNote : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.saveNoteButton.setOnClickListener {
            ...
            Log.e("AddNoteDebug", "Name: $name Title: $title, Description: $description, Class: $studentClass")
            ...
            if (title.isEmpty() || description.isEmpty() || studentClass.isEmpty()) {
                ...
                currentUser?.let { user ->
                    ...
                    // note item instance
                    val noteItem = NoteItem(name, studentClass, title, description, noteKey ?: "", userId)
                    if (noteKey != null) {
                        ...
                        databaseReference.child("users").child(user.uid).child("notes")
                            ...
                            .addOnCompleteListener { task ->
                                if (task.isSuccessful) {
                                    ...
                                    Log.e("AddNotePath", "Saved at path: users/${user.uid}/notes/$noteKey")
                                    Log.e("AddNoteUser", "Saved by user: ${user.email ?: "Unknown Email"} (UID: ${user.uid})")
                                    Log.e("AddNoteData", "Saved NoteItem: $noteItem")
                                    Toast.makeText(this, "Note Save Successful", Toast.LENGTH_SHORT)
                                        .show()
                                    finish()
                                } else {
                                    Log.e("AddNoteError", "Error saving note: ${task.exception?.message}")
